...
class Place(object):
    ...
    def remove_insect(self, insect):
        ...
        if insect.is_ant:
            ...
            if insect.__class__ is QueenAnt and insect.is_queen_ant:
                return

            elif insect.__class__ is BodyguardAnt and insect.name is 'Bodyguard' and insect.ant is not None:
...
class Bee(Insect):
    ...
    def action(self, colony):
        ...
        if self.blocked():
            ...
            if self.place.ant.name is 'Queen' and not self.place.ant.is_queen_ant:
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        bee_places = self.place

        while bee_places is not hive:

            if bee_places.bees == []:
                bee_places = bee_places.entrance

            else:
                return random_or_none(bee_places.bees)
                break
...
class LongThrower(ThrowerAnt):
 ...
 # min_range
    def nearest_bee(self, hive):
        ...
        if self.min_range != None:
            bee_places = self.place

            while self.min_range > 0:
                bee_places = bee_places.entrance
                self.min_range -= 1
...
class ShortThrower(ThrowerAnt):
    ...
    def nearest_bee(self,hive):
        ...
        # max_range
        bee_places = self.place.entrance
        if self.max_range != None:
            while self.max_range > 0 and bee_places is not hive:
...
class QueenAnt(ScubaThrower):  # You should change this line
    ...
    def __init__(self):
        # BEGIN Problem 9
        Ant.__init__(self, 1)
        QueenAnt.num_counts += 1

        if QueenAnt.num_counts == 1:
            self.is_queen_ant = True

        self.list_of_empty_containers_places = []
    ...
    def action(self, colony):
        ...
        def double(previous_place):
            
            
            while previous_place != None:

                ant_at_previous_place = previous_place.ant
                
                if ant_at_previous_place is not None and ant_at_previous_place.container and ant_at_previous_place.ant != None and ant_at_previous_place.ant.doubled is False:

                        ant_at_previous_place.ant.doubled = True
                        ant_at_previous_place.ant.damage *= 2

                if ant_at_previous_place is not self and ant_at_previous_place != None and ant_at_previous_place.doubled is False:

                    ant_at_previous_place.damage *= 2
                    ant_at_previous_place.doubled = True
                
                previous_place = previous_place.exit


        
        if not self.is_queen_ant:
