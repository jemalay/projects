...
def make_call_frame(procedure, args, env):
    ...
    if(isinstance(procedure,MuProcedure)):
        return Frame.make_child_frame(env,procedure.formals,args)
        
    return Frame.make_child_frame(procedure.env,procedure.formals,args)
...
class Frame:
    ...
    def lookup(self, symbol):
        ...
        if(self.parent is not None):
            for x in self.parent.bindings:
                if x == symbol:
                    return self.parent.bindings[x]
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        Frame.define(env, expressions.first,(scheme_eval(expressions.second.first,env)))
        return expressions.first

        # END Question 5A
    elif isinstance(target, Pair) and scheme_symbolp(target.first):
        # BEGIN Question 9A
        Frame.define(env,target.first,do_lambda_form(Pair(target.second, expressions.second),env))
...
def do_and_form(expressions, env):
    ...
    if(expressions is nil):
        ...
        return scheme_eval(expressions.first,env)
    elif(scheme_eval(expressions.first,env) is False):
        return False
    else:
...
def do_or_form(expressions, env):
    ...
    if(expressions is nil):
        return False
    elif(scheme_true(scheme_eval(expressions.first,env))):
        return scheme_eval(expressions.first,env)
    else:
...
def make_let_frame(bindings, env):
    ...
    parameters, arguments = nil ,nil

    while bindings:
        
        parameters = Pair(bindings.first.first,parameters)
        arguments = Pair(scheme_eval(bindings.first.second.first,env),arguments)
    ...
    return Frame.make_child_frame(env,parameters,arguments)
...
def check_formals(formals):
    ...
    while formals:
        ...
        if not scheme_symbolp(formals.first):
            raise SchemeError
        if formals.first in check_same_items:
            raise SchemeError
        check_same_items.append(formals.first)
        formals = formals.second
