...
def constructBayesNet(gameState):
    ...
    for house in HOUSE_VARS:
        ...
        for pos in [X_POS_VAR, Y_POS_VAR]:
            edges.append((pos, house))
        ...
        for obsPos in gameState.getHouseWalls(housePos):
            ...
            for assignment in obsFactor.getAllPossibleAssignmentDicts():
                ...
                if assignment[FOOD_HOUSE_VAR] == val:
                    ...
                    if assignment[obsVar] != 'blue' and assignment[obsVar] != 'red':
...
def getMostLikelyFoodHousePosition(evidence, bayesNet, eliminationOrder):
    ...
    """
    "*** YOUR CODE HERE ***"
    factors = inference.inferenceByVariableElimination(bayesNet, [FOOD_HOUSE_VAR], evidence, eliminationOrder)
    locationDict = {}
    for assign in factors.getAllPossibleAssignmentDicts():
        locationDict[factors.getProbability(assign)] = assign['foodHouse']
    return {'foodHouse': locationDict[max(locationDict.keys())]}
...
class VPIAgent(BayesAgent):
    ...
    def computeEnterValues(self, evidence, eliminationOrder):
        ...
        *Do not* take into account the "time elapsed" cost of traveling to each
        ...
        factor = inference.inferenceByVariableElimination(self.bayesNet, [GHOST_HOUSE_VAR,FOOD_HOUSE_VAR], evidence, eliminationOrder)
        for assignment in factor.getAllPossibleAssignmentDicts():
            if assignment[FOOD_HOUSE_VAR] == TOP_LEFT_VAL and assignment[GHOST_HOUSE_VAR] == TOP_RIGHT_VAL:
                
                left = factor.getProbability(assignment)
                
            elif assignment[FOOD_HOUSE_VAR] == TOP_RIGHT_VAL and assignment[GHOST_HOUSE_VAR] == TOP_LEFT_VAL:
                
                right = factor.getProbability(assignment)
               
        leftExpectedValue = GHOST_COLLISION_REWARD*right + WON_GAME_REWARD*left
        rightExpectedValue = GHOST_COLLISION_REWARD*left + WON_GAME_REWARD*right
    ...
    def computeExploreValue(self, evidence, enterEliminationOrder):
        ...
        "*** YOUR CODE HERE ***"
        # factor = inference.inferenceByVariableElimination(self.bayesNet, [GHOST_HOUSE_VAR,FOOD_HOUSE_VAR], evidence, enterEliminationOrder)

        for x,y in self.getExplorationProbsAndOutcomes(evidence):
            leftExpectedValue, rightExpectedValue = self.computeEnterValues(y, enterEliminationOrder)
            expectedValue += x*max(leftExpectedValue, rightExpectedValue)
