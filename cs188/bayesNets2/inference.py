...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet, queryVariables, evidenceDict, eliminationOrder):
        ...
        for hidden in eliminationOrder:
            factors, joined = joinFactorsByVariable(all_factors, hidden)
            all_factors = factors

            if len(joined.unconditionedVariables()) > 1:
                elim = eliminate(joined, hidden)
                all_factors.append(elim)

        all_factors = joinFactors(all_factors)
        all_factors = normalize(all_factors)
        return all_factors
